...
- **P6.2 Provision + deploy** — **blocked: requires external accounts (Qdrant Cloud, Redis Cloud, Google Cloud) this agent cannot create.** Everything else is ready: the Dockerfile builds and runs correctly, env vars are all config-driven already (`QDRANT_URL`/`QDRANT_API_KEY`, `REDIS_URL`, `OPENAI_API_KEY`, `GROQ_API_KEY`, `LANGFUSE_*`, `CORS_ALLOWED_ORIGINS`, `RATE_LIMIT_PER_MINUTE`). Remaining steps once accounts exist: create the Qdrant/Redis free instances, re-run `data.pipeline` (or a Qdrant snapshot restore) against them, deploy via `gcloud run deploy --source .` (builds directly from the existing Dockerfile via Cloud Build, no separate registry push needed) with those as env vars, verify `/health` live over the public Cloud Run URL. **Loose end closed while in the area**: `docker-compose.yml`'s `langfuse`/`langfuse-db` services (self-hosted Langfuse, dead since the P3 replan moved to Langfuse Cloud but never actually deleted) — flagged again here, not yet removed, low priority relative to the account-creation blocker.
...
- **P6.5 Real re-benchmark on the actual deployment target** — **blocked on P6.2** (needs the real x86 HF Spaces host, or at minimum genuine x86 hardware — a QEMU-emulated local Docker run was considered but rejected: emulation overhead could badly skew a CPU-bound benchmark's absolute numbers, and a misleading "verified" number would be worse than an honestly-still-open one). P2.3's finding stands as Apple-Silicon-only until this runs for real.
...
- **Done when:** public URL (Vercel) streams cited answers end-to-end from the live HF Spaces backend + Qdrant Cloud + Redis Cloud; cold + warm latency documented (both HF Spaces' own cold start and the semantic-cache hit/miss split); reranker_mode re-benchmarked and confirmed correct for the real x86 host, not carried over from Apple Silicon; demo verified stable under a few concurrent users; $0/month recurring cost. **P6.2 and P6.5 remain open, blocked on external account creation this agent cannot do — everything else (P6.1/P6.3/P6.4/P6.6) is built, real-verified, and ready to deploy the moment those accounts exist.**
